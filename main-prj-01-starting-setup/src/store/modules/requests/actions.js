export default {
  async contactCoach(context, payload) {
    ...
    const response = await fetch(
      `https://vue-demo-d9347-default-rtdb.firebaseio.com/requests/${payload.coachId}.json`,
  ...
  async fetchRequests(context) {
    ...
    const response = await fetch(
      `https://vue-demo-d9347-default-rtdb.firebaseio.com/requests/${coachId}.json?auth=${token}`
    );
    const data = await response.json();
